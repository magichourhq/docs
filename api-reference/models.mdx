...
<Note>
  ...
  resolves to `ltx-2.3`, and shorter output resolves to `kling-3.0`. Free typically requests `480p`;
  paid plans can request the higher resolutions a model supports, at that resolution's credit cost.
  The deprecated `kling-2.5-audio` and `veo3.1-audio` values are equivalent to passing the base model
  with `audio: true`.
</Note>
...
| [Lip Sync](/api-reference/video-projects/lip-sync)                               | 1 credit/frame on `standard`; 2 on paid-only `pro` |
