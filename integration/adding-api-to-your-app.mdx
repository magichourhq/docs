...
- API key from [Developer Hub](https://magichour.ai/developer?tab=api-keys&ref=docs-adding-api-to-your-app&utm_source=docs&utm_medium=referral&utm_campaign=adding-api-to-your-app)
...
The response contains the project `id` and `credits_charged`. Retrieve project details to check its status.
...
print(f"Job created with ID: {create_res.id}, credits: {create_res.credits_charged}")
```
...
<Card title="Inputs and Outputs Guide" icon="file" href="/integration/inputs-and-outputs">
...
try:
    ...
    while True:
        ...
        if status_res.status == "complete":
            ...
            break
        elif status_res.status in ("error", "canceled"):
            error_code = getattr(status_res.error, "code", "unknown")
            error_msg = getattr(status_res.error, "message", "Unknown error")
...
try {
  ...
  while (true) {
    ...
    if (statusRes.status === "complete") {
      // Success - download result
      break;
    } else if (statusRes.status === "error" || statusRes.status === "canceled") {
...
```json
{
  ...
  "downloads": [
    {
      "url": "https://videos.magichour.ai/id/output.mp4?auth-token=1234",
