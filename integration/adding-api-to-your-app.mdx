...
<Warning>
  Do not expose your API key in client-side code. Keep it secure on your server
  to prevent unauthorized usage and potential abuse.
</Warning>
...
Video and images can be one of the following statuses:
...
This is the recommended way to receive status change events; however, it does require extra effort to setup.
...
while True:
    res = client.v1.video_projects.delete(id=project_id)
...
Once `status` is `complete`, the response for the image/video details will populate the `downloads` key. Note this is an array since some modes may have more than one outputs
...
By default, all generated content are kept in storage without expiration. If you want to remove the generated files, you can use the following endpoints:
...
let res = client.v1().video_projects().delete(
  magic_hour::resources::v1::video_projects::DeleteRequest { id: "cm6pvghix03bvyz0zwash6noj".to_string() }
).await;
```
...
<Info>
  The mock server only responds with sample data and does not actually create
  any records.
</Info>
