...
- **Video projects:** `GET /v1/video-projects/{id}`
- **Image projects:** `GET /v1/image-projects/{id}`
- **Audio projects:** `GET /v1/audio-projects/{id}`
- **Webhooks:** Real-time notifications (recommended for production)

**Returns:** Current status (`queued`, `rendering`, `complete`, `error`)

### Step 3: Download Results

**Purpose:** Retrieve your generated content

**Method:** Use download URLs from the status response

- URLs are time-limited (24 hours)
- No special API endpoints needed - direct HTTP download

## SDK Functions: `create` vs `generate`

**`create()` function:**

- Returns immediately with project ID
- Requires manual polling or webhooks
- Best for production apps with multiple concurrent jobs
- Available in all SDKs

**`generate()` function:**

- Handles polling and downloading automatically
- Blocks until completion
- Best for simple scripts and prototyping
- Available in Python SDK v0.36.0+ and Node SDK v0.37.0+

```python
# create() - Manual workflow
create_res = client.v1.face_swap.create(...)
# You handle polling and downloading

# generate() - Automatic workflow
result = client.v1.face_swap.generate(..., wait_for_completion=True)
# SDK handles everything
```

## Project structure

Here's a recommended project structure for Magic Hour API integration:

```
my-app/
├── src/
│   ├── services/
│   │   ├── magichour.js          # API client setup
│   │   ├── video-processor.js    # Video generation logic
│   │   └── file-manager.js       # Upload/download handling
│   ├── models/
│   │   └── job.js               # Job status tracking
│   └── utils/
│       ├── polling.js           # Smart polling logic
...
- **API Client:** Centralized Magic Hour client configuration
- **Service Layer:** Business logic for different content types
- **File Management:** Handle uploads, downloads, and cleanup
- **Job Tracking:** Monitor multiple concurrent jobs
- **Webhook Handlers:** Real-time status updates

## Integration patterns

Choose the right pattern based on your application needs:

### Pattern 1: Simple Script (Quick Start)

**Best for:** Prototypes, one-off tasks, learning

```python
# Use generate() for simplicity
