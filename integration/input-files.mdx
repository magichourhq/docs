...
<Tip>
  If requesting multiple upload urls, the response array will be in the same order as the request
  body.
</Tip>
...
fetch("https://videos.magichour.ai/api-assets/id/video.mp4?auth-value=1234567890", {
  method: "PUT",
  headers: { "Content-Type": "application/octet-stream" },
  body: file,
});
