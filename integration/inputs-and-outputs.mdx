...
<Tip>
  Pre-signed URLs work when the URL itself grants access to the file. Magic Hour
  cannot forward your browser session, cookies, or custom authorization headers.
</Tip>

### URL Requirements

- Must use a public or pre-signed URL that returns file bytes without a login page
...
<Info>
  **Library Access**: You can only use library URLs for files in your own Magic Hour account. The
  file IDs are available from your [my-library
  dashboard](https://magichour.ai/my-library?utm_source=docs&utm_medium=referral&utm_campaign=inputs-and-outputs)
  or from API responses.
</Info>
...
func main() {
	...
	uploadResponse, err := fileClient.Do(req)
	if err != nil {
		fmt.Println(err)
		return
	}
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let mut client = magic_hour::Client::default()
        .with_bearer_auth(&std::env::var("MAGIC_HOUR_API_KEY")?);

    // Step 1: Request upload URL
    let response = client.v1().files().upload_urls()
        .create(magic_hour::resources::v1::files::upload_urls::CreateRequest {
            items: vec![magic_hour::models::V1FilesUploadUrlsCreateBodyItemsItem {
                extension: "mp4".to_string(),
                type_: magic_hour::models::V1FilesUploadUrlsCreateBodyItemsItemTypeEnum::Video,
            }],
        }).await?;

    // Step 2: Upload raw bytes to the signed URL without the API bearer token
    let bytes = std::fs::read("/path/to/file/video.mp4")?;
    reqwest::Client::new()
...
```bash cURL
# Step 1: Request upload URL
response=$(curl -fsS https://api.magichour.ai/v1/files/upload-urls \
  --request POST \
...
```json
{
  "items": [
    {
      "upload_url": "https://videos.magichour.ai/api-assets/id/video.mp4?<signed-query-params>",
...
# Upload each file to its corresponding upload_url before using these paths.
# Requesting URLs alone does not upload any bytes; order matches the request.
...
// Upload each file to its corresponding uploadUrl before using these paths.
// Requesting URLs alone does not upload any bytes; order matches the request.
...
**Web Dashboard**: All generated content appears in your [magichour.ai/my-library](https://magichour.ai/my-library?utm_source=docs&utm_medium=referral&utm_campaign=inputs-and-outputs) dashboard.
