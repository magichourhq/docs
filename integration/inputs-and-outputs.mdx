...
Magic Hour APIs require input files (images, videos, audio) and produce output files (generated content). This guide covers both sides of file handling: getting your files to Magic Hour and retrieving the results.
...
<Info>
**Library Access**: You can only use library URLs for files in your own Magic Hour account. The file IDs are available from your [my-library dashboard](https://magichour.ai/my-library) or from API responses.
</Info>
...
1. Request upload URLs specifying file extension and type
2. Upload files to the provided URLs via PUT request
3. Use the returned `file_path` in your API calls
...
const result = await client.v1.faceSwapPhoto.create({
  assets: {
    ...
    targetFilePath: "https://upload.wikimedia.org/wikipedia/commons/e/ec/Chris_Cassidy_-_Official_NASA_Astronaut_Portrait_in_EMU_%28cropped%29.jpg"
  }
});
```
...
**Best practices:**
- Upload files just before using them in API calls
- Reuse the same `file_path` for multiple API calls within 7 days
- For permanent storage needs, contact [support@magichour.ai](mailto:support@magichour.ai)

<Tip>
**Reuse Uploads**: You can reference the same `file_path` in multiple API calls before it expires, avoiding duplicate uploads.
</Tip>
...
For production applications, implement robust download logic:
...
result = client.v1.ai_image_generator.generate(
    ...
    download_outputs=True,              # Auto-download
    download_directory="./outputs/"     # Where to save
)
...
- **Reuse**: Can be used in multiple API calls within 7 days
...
# Use in multiple API calls within 7 days
...
- **Storage**: Stored indefinitely in Magic Hour cloud storage
- **Download URLs**: Expire after 24 hours
- **Accessing**: Retrieve fresh URLs anytime using GET endpoints
- **Web App**: All generated content appears in your [magichour.ai/my-library](https://magichour.ai/my-library) dashboard
