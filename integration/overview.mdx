...
Processing time varies significantly by endpoint, model, settings, input, and queue load. Build
status monitoring and choose timeouts for your own workload instead of relying on a fixed estimate.

<Card
  title="Plan status monitoring and timeouts"
  icon="clock"
  href="/api-reference/processing-times"
>
  Use webhooks or polling with backoff, handle terminal states, and choose workload-specific
  timeouts.
</Card>
