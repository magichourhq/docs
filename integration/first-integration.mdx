...
<Note>
  **Video processing:** Video jobs run asynchronously, and their duration varies with the input,
  selected settings, and queue load. Poll with exponential backoff or use webhooks instead of
  assuming a fixed completion time.
</Note>
