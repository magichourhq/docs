...
If you have not made a creation yet, start with the shorter [API quickstart](/get-started/quick-start). This tutorial explains manual polling and file handling after that first success.
...
<Note>
  **Estimated time**: 15-20 minutes **Prerequisites**: API key from [Developer
  Hub](https://magichour.ai/developer?tab=api-keys&ref=docs-first-integration&utm_source=docs&utm_medium=referral&utm_campaign=first-integration),
  Python or a current Node.js LTS installed
</Note>
...
<Info>
  **SDK Documentation**: Full Python SDK docs available at
  [github.com/magichourhq/magic-hour-python](https://github.com/magichourhq/magic-hour-python). The
  Python package is published as `magic_hour` on PyPI; the Node.js SDK is `magic-hour` on npm.
</Info>
...
def main():
    ...
    print("   Waiting for the image render")
    ...
    while attempt < max_attempts:
        try:
            ...
            if status == "complete":
                ...
                print(f"   Code: {getattr(error_info, 'code', 'unknown')}")
                print(f"   Message: {getattr(error_info, 'message', 'No error message')}")
                return
    ...
    if attempt >= max_attempts:
        print(f"Wait timed out for project {job_id}. Check this project again; do not create a duplicate.")
        return
...
async function main() {
  ...
  try {
    ...
    // Step 2: Poll for completion
    console.log("\n⏳ Waiting for job to complete...");
    console.log("   Waiting for the image render");
    ...
    if (attempt >= maxAttempts) {
      console.log(
        `Wait timed out for project ${jobId}. Check this project again; do not create a duplicate.`
      );
      return;
    }
...
console.log(`✅ Complete! Files: ${result.downloadedPaths}`);
```
...
<Note>
  **Video processing:** Video jobs run asynchronously. Review the recent [typical processing
  times](/api-reference/processing-times) for planning, then poll with exponential backoff or use
