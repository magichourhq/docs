...
- ✅ **Handle retries gracefully** (deduplicate repeated deliveries)
...
<Warning>
  These examples omit signature verification. Before accepting events,
  [verify the signature and timestamp](/integration/webhook/secure-handler) against the raw body.
  Save the verified event to a database or durable queue before returning 2xx. A restart can lose
  in-process background tasks after Magic Hour considers delivery complete.
</Warning>
...
### 1. Deduplicate Webhook Deliveries

Magic Hour may retry delivery, so detect and skip repeated events:
...
Use an atomic database claim or unique constraint for the deduplication key so concurrent deliveries
cannot both process it. Apply job-state changes transactionally and avoid replacing a terminal state
with a delayed `started` event.
...
- Deduplicate deliveries using the project ID and event type
- Store processed deduplication keys
