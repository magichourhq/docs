...
- ❌ **Malicious payloads** attempting to exploit your system  
- ❌ **Spam requests** overwhelming your servers
- ❌ **Data corruption** from processing invalid events

**✅ Solution**: Verify that webhooks are actually from Magic Hour using cryptographic signatures.

<Warning>
**Production Requirement**: Always verify webhook signatures in production environments. This is critical for security.
</Warning>
...
sequenceDiagram
    ...
    App->>App: 8. Compare signatures → Accept/Reject
```
...
<Info>
**Header Case**: HTTP headers are case-insensitive, so `Magic-Hour-Event-Signature` and `magic-hour-event-signature` are equivalent.
</Info>
