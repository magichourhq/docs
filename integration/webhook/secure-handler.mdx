...
WEBHOOK_SECRET = os.environ["MAGIC_HOUR_WEBHOOK_SECRET"]
...
app.post("/webhook", (req, res) => {
  ...
  // Reject malformed timestamps and deliveries outside the five-minute window.
  const sentAt = Number(timestamp);
  if (
    typeof timestamp !== "string" ||
    !/^\d+$/.test(timestamp) ||
    !Number.isSafeInteger(sentAt) ||
    Math.abs(Date.now() / 1000 - sentAt) > 300
  ) {
...
```bash cURL Test
# Negative test: this placeholder signature must be rejected (401)
