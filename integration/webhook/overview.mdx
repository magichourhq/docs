...
print("💡 Use an HTTPS tunneling provider to expose port 8000, or use webhook.site")
```
...
<Tabs>
<Tab title="HTTPS Tunnel (Recommended)">

Use any HTTPS tunneling provider that can forward a public URL to your local server.
...
Follow your provider's setup instructions and forward its public HTTPS URL to `http://localhost:8000`.

**Copy the HTTPS URL** and append `/webhook` when registering it with Magic Hour.
...
3. **Get your public URL:**
   - Use an HTTPS tunneling provider that supports notebook environments
   - Forward the public URL to port `8000`
   - Append `/webhook` to the public URL

4. **Use the URL** in Magic Hour webhook setup

**Note**: Temporary tunnel URLs may change when restarted. Use a persistent URL for production.
...
Go to Magic Hour [Developer Hub](https://magichour.ai/developer?tab=webhooks&ref=docs-webhook-overview&utm_source=docs&utm_medium=referral&utm_campaign=webhook-overview), and click **Create Webhook**

![Developer Hub Webhooks tab](./images/webhooks-current.jpg)
...
- **Endpoint URL**: Your public HTTPS URL (e.g., `https://your-tunnel.example/webhook`)
...
- ✅ Install required packages: `!pip install fastapi uvicorn nest-asyncio`
- ✅ Make sure the server thread started successfully
- ✅ Check if your HTTPS tunnel is active and accessible
