...
<Note>
  **Using Jupyter/Colab?** The server examples below are for standalone deployment. For notebook
  testing: 1. Use [webhook.site](https://webhook.site) for instant webhook testing (no code needed)
  - see the "webhook.site" tab in Step 2 2. Or use the notebook-friendly code in the "Colab/Jupyter"
  tab below
</Note>
