...
<Note>
  **Using Jupyter/Colab?** See the "Colab/Jupyter" tab below for notebook-compatible code, or use
  [webhook.site](https://webhook.site) for instant testing without any code.
</Note>
...
Go to Magic Hour [Developer Hub](https://magichour.ai/developer?tab=webhooks&ref=docs-create-webhook&utm_source=docs&utm_medium=referral&utm_campaign=webhook-create-webhook), and click **Create Webhook**
...
After the project changes status, you should see a webhook event in your console or webhook.site.
This example abbreviates the project payload; use the [webhook
reference](/webhook-reference/image-events/image-completed) for its complete schema.
...
- **Duration**: Until delivery succeeds or the event is 24 hours old
- **Pattern**: Exponential backoff (1, 2, 4, ... minutes), capped at 60 minutes
- **Expiration**: Once an event is more than 24 hours old, the next delivery pass marks it failed

<Info>
  **Disabled webhooks**: Disabling a webhook stops new events from being queued. Events that are
  already pending are rescheduled until you re-enable the webhook or their 24-hour delivery window
  expires.
</Info>
...
- Return a 2xx status promptly
- Process events asynchronously (respond fast, process later)
- Deduplicate repeated webhook deliveries
- Log project IDs, event types, and outcomes without logging secrets or raw payloads
