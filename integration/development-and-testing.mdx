...
The Magic Hour SDKs can use a hosted mock server that returns sample data without processing jobs or charging credits:
...
The mock server validates SDK wiring, not real generation quality, account eligibility, credit
deductions, or webhook delivery. Its sample project is not a real job; do not wait for it to
progress through rendering. Verify those behaviors separately against the real API.
...
```python Python
from magic_hour import ApiError, Client
...
try:
    ...
    if code == 401:
        ...
        print("❌ Payment required. Add credits, subscribe, or upgrade the plan named by the error code.")
    elif code in (400, 422):
        print(f"❌ Invalid parameters: {detail}")
    elif code == 404:
        print("❌ Route or project not found. Check the resource ID.")
    elif code >= 500:
        print("❌ Server error. Check for an accepted project before retrying this creation request.")
    else:
...
try {
  ...
  if (statusCode === 401) {
    ...
    console.log(
      "❌ Payment required. Add credits, subscribe, or upgrade the plan named by the error code."
    );
  } else if (statusCode === 400 || statusCode === 422) {
    console.log(`❌ Invalid parameters: ${error.message}`);
    // Fix your request parameters
  } else if (statusCode === 404) {
    console.log("❌ Route or project not found. Check the resource ID.");
  } else if (statusCode >= 500) {
    console.log(
      "❌ Server error. Check for an accepted project before retrying this creation request."
    );
  } else {
...
while time.monotonic() < deadline:
    ...
    raise TimeoutError(f"Job {job_id} is still pending. Resume polling this ID; do not submit a new job.")
...
while (Date.now() < deadline) {
  ...
  if (status.status === "complete") {
    ...
    downloadUrl = status.downloads[0].url;
    break;
    ...
    throw new Error(`Job ${jobId}: ${status.status}`);
  }
...
if (!downloadUrl) {
  throw new Error(
    `Job ${jobId} is still pending. Resume polling this ID; do not submit a new job.`
  );
}
```
...
<Note>
  Creation charges credits up front. If generation fails, those credits are refunded. A client-side
  timeout does not cancel the job or trigger a refund.
</Note>

### Retry without creating duplicate jobs

- **Before receiving a project ID:** a network timeout or server error does not prove the creation
  request failed. Check your project history before submitting again. Do not assume repeated POST
  requests are deduplicated, or that sending an `Idempotency-Key` header makes them safe.
- **After receiving a project ID:** save it immediately. Retry status reads for that same ID with
  bounded exponential backoff; do not create a replacement job just because polling timed out.
- **After completion:** retry downloading the existing output. If its signed URL expired, retrieve
  the project again for a current download URL instead of paying for another generation.
- **After `error` or `canceled`:** stop polling. Fix the reported input or account problem before
  creating a new job. Cancellation does not emit a webhook, so keep polling as a recovery path.
...
<Steps>
  ...
  <Step title="Cancellation complete">
    ![Cancel Success](./images/cancel-3.jpg) Full credit refund is provided for cancelled video
    jobs.
  </Step>
</Steps>
...
- Check the current status and compare the job age with the endpoint's recent
  [typical processing time](/api-reference/processing-times) and your workload-specific timeout; the
  typical time is not a failure threshold
