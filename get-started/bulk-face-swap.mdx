...
runners below call a tested single-photo script sequentially, wait for each result, and stop on the
...
<CardGroup cols={2}>
  <Card title="Face Swap API page" icon="code" href="https://magichour.ai/api/face-swap">
    Copy a complete Python, JavaScript, or curl request-to-result example
  </Card>
  <Card
...
2. Open the [Face Swap API page](https://magichour.ai/api/face-swap), choose a language, and save its
   single-photo example as `example.py`, `example.mjs`, or `example.sh`.
...
```python Python
# Save the Face Swap API page's single-photo Python example as example.py first.
...
// Save the Face Swap API page's single-photo JavaScript example as example.mjs first.
...
```bash curl / Bash
#!/usr/bin/env bash
# Save the Face Swap API page's single-photo curl example as example.sh first.
...
log has no project ID after a timeout, check the Developer Hub before submitting again. Preserve the
original logs; deleting them and rerunning the whole list can create duplicate charges.
