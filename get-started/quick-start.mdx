...
print(
    f"queued face swap video with id {create_res.id}, spent an estimated {create_res.estimated_frame_cost} frames based on 30fps. This value be updated after render completes if fps is different."
)
...
while True:
    ...
    if res.status == "complete":
        print(f"render complete! Final frame cost is {res.total_frame_cost}, actual fps is {res.fps}.")
        with (
            urllib.request.urlopen(res.downloads[0].url) as response,
            open(output_file, "wb") as out_file,
        ):
...
const client = new Client({
  token: "YOUR_API_KEY", // change to your API key
});
...
async function main() {
  ...
  console.log(
    `queued face swap video with id ${createRes.id}, spent an estimated ${createRes.estimatedFrameCost} frames based on 30fps. This value be updated after render completes if fps is different.`
  );
  ...
  while (true) {
    ...
    if (res.status === "complete") {
      console.log(
        `render complete! Final frame cost is ${res.totalFrameCost}, actual fps is ${res.fps}.`
      );
...
func main() {
	...
	fmt.Printf("queued video with id %s, spent an estimated %d frames based on 30fps. This value be adjusted after render completes if fps is different.\n", createRes.Id, createRes.EstimatedFrameCost)
	...
	for {
		...
		if res.Status == "complete" {
			fmt.Printf("render complete! Final frame cost is %d, actual fps is %f.\n", res.TotalFrameCost, res.Fps)
			url := res.Downloads[0].Url
...
pub async fn main() {
    ...
    let create_res = client
        ...
        .unwrap();
    let project_id = create_res.id;
    let frame_cost = create_res.estimated_frame_cost;
    println!("queued face swap video with id {project_id}, spent an estimated {frame_cost} frames based on 30fps. This value be updated after render completes if fps is different.");
    ...
    loop {
        ...
        match res.status {
            magic_hour::models::GetV1VideoProjectsIdResponseStatusEnum::Complete => {
                println!(
                    "render complete! Final frame cost is {}, actual fps is {}",
                    res.total_frame_cost, res.fps
                );
                let url = res.downloads[0].url.clone();
...
project_id=$(echo $create_response | jq -r '.id')
estimated_frame_cost=$(echo $create_response | jq -r '.estimated_frame_cost')

echo "queued face swap video with id $project_id, spent an estimated $estimated_frame_cost frames based on 30fps. This value be adjusted after render completes if fps is different."
while true; do
    status_response=$(curl -s $STATUS_URL/$project_id --header "Authorization: Bearer $API_KEY")
    ...
    if [ "$status" == "complete" ]; then
        total_frame_cost=$(echo $status_response | jq -r '.total_frame_cost')
        fps=$(echo $status_response | jq -r '.fps')
        download_url=$(echo $status_response | jq -r '.downloads[0].url')
        echo "render complete! Final frame cost is $total_frame_cost, actual fps is $fps."
