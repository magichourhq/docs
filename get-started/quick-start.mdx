...
- `download_directory: "./outputs"` - Save to the outputs directory
...
result = client.v1.ai_image_generator.generate(
    ...
    download_directory="./outputs", # save to the outputs directory we created earlier
)
...
async function main() {
  const createRes = await client.v1.aiImageGenerator.generate(
    ...
    },
    {
      ...
      downloadDirectory: "./outputs", // save to the outputs directory we created earlier
    }
  );
    ...
    loop {
        ...
        match res.status {
            magic_hour::models::GetV1VideoProjectsIdResponseStatusEnum::Complete => {
                ...
                tokio::task::block_in_place(move || {
                    let response = reqwest::blocking::get(url).unwrap();
                    let output_path = "./outputs/output.mp4";
                    if response.status().is_success() {
                        let mut output_file = std::fs::File::create(output_path).unwrap();
                        std::io::copy(&mut response, &mut output_file).unwrap();
                        println!("file downloaded successfully to {}", output_path);
