...
1. Name the key, then click `Create key`.
   ![Create API Key Modal](./images/api-key-2.png)
1. Copy the key and save it.
...
<Note>SDKs reduce boilerplate. Skip to the Step 3 if you're not planning on using our SDKs.</Note>
...
{/* <Note>
  The `.generate` function is only available in the Python SDK v0.36.0 or later. We are working on
  adding it to the other SDKs.
</Note> */}
...
<Check>🎉 Congratulations! You've generated your first asset with Magic Hour!</Check>
...
<Info>💡 Tip: For production apps, set your `MAGIC_HOUR_API_KEY` as an environment variable and enable webhooks for better reliability.</Info>
<Tip> Prefer fast iteration inside your editor? Install this documentation as an MCP server to get contextual help while integrating the Magic Hour API. [Learn more](./model-context-protocol). </Tip> 