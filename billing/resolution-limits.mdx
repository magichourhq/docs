...
[image to video](/api-reference/video-projects/image-to-video) take `360p`, `480p`, `720p`, `1080p`, or `4k`, depending on the model.
...
Creator, Pro, and Business also differ on credits per month, upload size, website concurrency, and support.
...
Use each endpoint's current resolution fields rather than applying a shared hard cap. For example,
[Talking Photo](/api-reference/video-projects/ai-talking-photo) accepts a plan-capped
`max_resolution`. Deprecated Video to Video `width` and `height` fields are no longer part of the
current request contract.

The [website pricing page](https://magichour.ai/pricing) summarizes plan export quality as
1024px, 1472px, and 4K. For API requests, use the endpoint schema and selected model
to determine which resolution parameter and limits apply.
