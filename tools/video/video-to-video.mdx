...
Choose a required `style.art_style`. Set `style.prompt_type` to `custom` to use your own prompt, or `append_default` to combine it with the preset prompt. The default prompt type ignores a supplied `style.prompt`.
...
Output resolution is managed by the service and your subscription limits. The `width` and `height` request fields are deprecated and no longer affect output resolution. See [Resolution Limits](/billing/resolution-limits).
...
<Card
  ...
  icon="webhook"
  href="/api-reference/video-projects/video-to-video"
>
...
<CardGroup cols={2}>
  <Card title="AI Video Editor" icon="wand-magic-sparkles" href="/tools/video/video-editor">
    Add, remove, or change video content with a prompt
  </Card>
