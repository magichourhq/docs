...
description: Transcribe speech and burn timed subtitles into a video.
---
...
Auto Subtitle Generator detects the spoken language, transcribes the audio, and burns timed subtitles into the output video. Transcription accuracy depends on the language, audio quality, background noise, and number of speakers.
...
1. Provide a video with spoken content.
2. Whisper detects the language and transcribes the speech.
3. The API times and formats the subtitle text.
4. Download the video with burned-in subtitles.
...
- **Accessibility workflows** - Generate a caption draft for human review
...
Auto Subtitle Generator uses Whisper for automatic language detection. Accuracy varies by language and recording quality.
...
Auto Subtitle Generator costs **0.2 credits per rendered frame**, so the total depends on the video
segment's duration and frame rate.

| Example            | Credits |
| :----------------- | ------: |
| 1 minute at 24 FPS |     288 |
| 1 minute at 30 FPS |     360 |
| 1 minute at 60 FPS |     720 |

<Info>
  The create response estimates cost at 30 FPS. After rendering, Magic Hour adjusts the charge for
  the rendered frame count. Check the completed project's `credits_charged` for the final cost.
</Info>
