...
The [product page](https://magichour.ai/products/auto-subtitle-generator) advertises support for 50+ languages. The API has no language-selection parameter. Review generated captions for names, accents, and technical terms, and choose a font that supports your language.
...
Retrieve the captioned video from the completed video project's `downloads` array. The current API request schema has no SRT/VTT export-format option; subtitle-track export features advertised on the website are not documented as API parameters.
