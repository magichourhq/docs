...
`assets.audio_file_path` and `end_seconds` are required. `start_seconds` defaults to zero; the reference image and prompt are optional. Supported output resolutions are `480p`, `720p`, and `1080p`, subject to your plan. The default is `480p` on the free tier and `720p` on paid tiers.
