...
Use `style.mode="replace"` to substitute the selected subject, or `"animate"` to transfer the source video's motion to the reference character. Subject selection defaults to `auto`. For manual selection, set `selection_mode="point"` and provide `points`; coordinates are pixels in the source frame, and timestamps use the source video's timeline. Points are ignored in automatic mode.

The API accepts `480p` and `720p`, subject to your plan, and defaults to `480p`. `end_seconds` is required; `start_seconds` defaults to zero.
