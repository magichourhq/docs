...
description: Replace one or more detected faces across frames in a video segment.
---
...
Face Swap Video replaces one or more detected faces across frames in a selected video segment,
depending on `face_swap_mode`. Fast motion, occlusion, lighting changes, and large angle differences
...
4. **API processes frame-by-frame** - The API replaces the selected faces across the processed frames
5. **Download the result** - Retrieve your face-swapped video

## Use Cases

- **Entertainment content** - Create social posts and memes
...
<Tip>
  **Use a clear source photo** - A larger, unobstructed face gives the model more detail to match.
</Tip>
...
<Card
  ...
  icon="webhook"
  href="/api-reference/video-projects/face-swap-video"
>
