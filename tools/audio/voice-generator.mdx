...
The [web tool](https://magichour.ai/products/ai-voice-generator) offers a large voice library and language selection. For API requests, use an exact `voice_name` value from the [API reference](/api-reference/audio-projects/ai-voice-generator), such as `Morgan Freeman`, `Joe Rogan`, or `Donald Trump`. Display labels on the website may be shortened.

The published API takes `prompt` and `voice_name`. It has no language parameter.
...
const project = await client.v1.aiVoiceGenerator.create({
  style: {
    prompt: "Hello! This is a test of the Magic Hour voice generator. Pretty cool, right?",
