...
description: Generate speech from text with a supported named voice.
---
...
AI Voice Generator converts text to speech with a named voice from the API's current voice list. Output varies with the selected voice, language, punctuation, and text.

<Warning>Use voices only when you have the rights and permissions required for your use.</Warning>
...
2. **Select a voice** - Choose a supported voice name
3. **API generates audio** - AI synthesizes the speech
...
- **Entertainment** - Create scripted character audio
...
<Note>
  Supported voice names can change. Check the [API
  reference](/api-reference/audio-projects/ai-voice-generator) for current values.
</Note>
...
Voice generation costs **0.1 credits per character** of text, rounded up to the nearest whole number. Inputs support up to 1,000 characters.
