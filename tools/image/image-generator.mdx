...
Set the top-level `model` to select a specific model, or use `default` to follow Magic Hour's recommendation as it changes. Choose `640px`, `1k`, `2k`, or `4k` for `resolution`, subject to your model and plan. `auto` is deprecated. `krea-2` supports `640px` and `1k` and one image per request. `gpt-image-2.5-flare` supports all four resolutions and 1 to 4 images per request.

`flux-2-klein` and `krea-2` accept only 1 image per request. `nano-banana-2` and `nano-banana-pro` accept 1, 4, 9, or 16. The other listed models accept 1 to 4.
...
| `gpt-image-2.5-flare` | 100                      | Creator, Pro, Business |
