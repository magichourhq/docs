...
Input images can be at most 4096×4096px. A 4x upscale requires Creator, Pro, or Business.
...
Set `style.mode` to `preserve` to retain details, `balanced` to enhance them, or `creative` to reinterpret them. The default is `balanced`. An optional `style.prompt` only affects `creative` mode. The legacy `pro` mode and `style.enhancement` field are deprecated.
