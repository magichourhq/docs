...
Without a replacement background, the API returns a PNG with a transparent background, which:
...
To replace the background in the same request, provide `assets.background_image_file_path`, or `backgroundImageFilePath` in Node.js. The API resizes the replacement to match the subject image, so use similar dimensions.
